...
import { History, Users, Play, Upload, Download, AlertCircle } from "lucide-react";
...
type GeneratedVideo = { url: string; prompt: string; model: string };
  ...
  async function handleGenerate() {
    ...
    try {
      const res = await fetch("/api/generate/video", {
        ...
        body: JSON.stringify({ prompt, model: model.id, duration, aspect }),
      });
            ...
            {history.length === 0 ? (
              <>
                ...
                <p className="text-[12px] text-white/40">
                  {tab === "history" ? "Your generated videos will appear here" : "Community videos from creators"}
                </p>
              </>
            ) : (
              <div className="w-full flex flex-col gap-2">
                {history.map((vid, i) => (
                  <div key={i} className="group relative overflow-hidden rounded-xl bg-black aspect-video">
        ...
        <main className="flex flex-1 flex-col items-center justify-center overflow-y-auto p-6">
          <div className="w-full max-w-[640px]">
            ...
            <div className="mb-5 flex gap-2">
              {[{ label: "Text to Video", icon: Play, val: "text" as const }, { label: "Image to Video", icon: Upload, val: "image" as const }].map(({ label, icon: Icon, val }) => (
                <button key={label} type="button" onClick={() => setMode(val)}
                  className={`flex items-center gap-1.5 rounded-lg px-4 py-2 text-[13px] font-semibold transition-colors ${
                    mode === val ? "bg-white/[0.1] text-hf-text" : "text-white/40 hover:text-white/60"
                  }`}>
                  <Icon size={13} />{label}
                </button>
              ))}
            ...
            {history.length > 0 && (
              <div className="mt-6">
                <video src={history[0].url} controls autoPlay className="w-full rounded-2xl" />
              </div>
            )}
          </div>
        </main>
